...
另附聊天框的可用命令（有些命令需要权限，`true`和`false`参数不分大小写，等价`on`和`off`）:
| 命令 | 仅房主可用 | 描述 |
|------|------|------|
| `/help <命令名>` | 否 | 获取对应命令的帮助 |
| `/tags (on|off|team)` | 是 | 是否启用玩家名牌，或仅为同队伍启用 |
| `/kill ...` | 是 | 击杀对应名称的玩家 |
| `/ban ...` | 是 | 将对应名称或id的玩家永久踢出房间 |
| `/unban ...` | 是 | 撤销`/ban` |
| `/hostteammemberchange <玩家名> (0|1|2)` | 是 | 为特定玩家设置队伍 |
| `/setallteam (0|1|2)` | 是 | 为所有玩家设置队伍 |
