...
iex($w.DownloadString('https://gitee.com/ravenfieldcommunity/UnionSetup/raw/master/get_mlang-utf8.txt'));
Read-Host;
```
备用链接：
```powershell
...
如果你的电脑是32位的，请到下方的Github下载`BepInEx_x86_XXX.zip`，即32位版本，因为32位比较冷门，所以蓝奏云社区分流处就懒得添加32位的BepInEX了
...
我们将翻译文件作为工坊物品上传是为了进行正版认证（毕竟Steam已经关闭了Mod下载工具对RF工坊访问权限）

如果您确有需要且无主观恶意，请自行找一个朋友代下
...
如果您不需要替换翻译（耗性能），则不要复制`main_regex-sch.txt`
...
`main_regex-sch.txt`是替换翻译文件，对应的特性消耗性能
